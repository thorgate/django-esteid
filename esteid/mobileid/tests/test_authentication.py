...
@pytest.mark.parametrize("hash_type", HASH_ALGORITHMS)
def test_authentication_manual_bytes(demo_mid_api, hash_type, MID_DEMO_PIN_EE_OK, MID_DEMO_PHONE_EE_OK):
    response_data = {"sessionID": "FAKE"}

    custom_random_data = b"is a custom random data which should be generated by os.urandom()"
    ...
    with patch.object(demo_mid_api, "invoke", return_value=response_data):
        ...
        assert res.session_id == "FAKE"
        assert res.hash_type == hash_type
        assert res.hash_value == expected_hash
        assert res.verification_code == get_verification_code(expected_hash)
